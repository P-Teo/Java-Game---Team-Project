...
/// extinde clasa Level pentru a face nivelul 1
...
public class Level1 extends Level {
    ...
    int maxEnemies = 12;
    List<Enemylvl1> enemies = new ArrayList<>(); // lista inamicilor activi din nivel
    int maxNowEnemies;
    ...
    public void update(boolean left, boolean right, boolean up, boolean down, boolean attack, int wndWidth, int wndHeight) {
        ...
        while (currentEnemyIndex < maxEnemies && absoluteX > 500 + currentEnemyIndex * 380) {
            ...
            if (currentEnemyIndex % 3 == 0) { newEnemy.x = -newEnemy.width - 10; } else { newEnemy.x = wndWidth + 10; }
        ...
        enemies.removeIf(e -> { if (e.isDead) { maxNowEnemies--; return true; } return false; });  // Elimină inamicii uciși
    ...
    public void draw(Graphics g) {
        ...
        for (Enemylvl1 enemy : enemies) { if(!showMessage) { enemy.draw(g); } }  // Desenează inamicii
        if (levelCompleted) {  game.nrLevel=2; drawLevelCompleteMessage(g); }  // Afișează mesajul de felicitări
