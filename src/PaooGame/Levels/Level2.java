...
/// extinde clasa Level pentru a face nivelul 2
...
public class Level2 extends Level {
    ...
    List<Enemylvl2> enemies = new ArrayList<>(); // lista inamicilor activi din nivel
    //List<Enemylvl2> enemyPool = new ArrayList<>(maxEnemies);
    ...
    public Level2(Game game, GameWindow wnd) {
        ...
        for(int i=0;i<maxTraps;i++){
            ...
            int trap_y = 140 + (int)(Math.random() * (510 - 140)); // Random pe verticală între 140 și 520
    ...
    public void update(boolean left, boolean right, boolean up, boolean down, boolean attack, int wndWidth, int wndHeight) {
        ...
        while (currentEnemyIndex < maxEnemies && absoluteX > 500 + currentEnemyIndex * 380) {
            Enemylvl2 newEnemy = new Enemylvl2();//enemyPool.remove(0);
            int randomY = 100 + (int)(Math.random() * 400);// Poziții random pe Y între 100 și 500
            newEnemy.y = randomY;
        ...
        for (Enemylvl2 enemy : enemies) {
            ...
            if (enemy.getIsAttacking() && areEntitiesColliding(player, enemy)) { player.takeDamage(enemy.damage); }
            // Dacă jucătorul atacă și e aproape, lovește inamicul
            if (attack&&!previousAttackState  && areEntitiesColliding(player, enemy)) { enemy.takeDamage(player.damage); previousAttackState = true; }
            else if (!attack) { previousAttackState = false; /* Permite atacul din nou dacă jucătorul nu mai atacă*/}
        }
        ...
        if (absoluteX > maxPlayerX) { maxPlayerX = absoluteX; score = maxPlayerX + (maxEnemies - maxNowEnemies) * 300; }
    }
    ...
    public void draw(Graphics g) {
        ...
        for (Enemylvl2 enemy : enemies) { if(!showMessage){  enemy.draw(g); } } // Desenează inamicii
        if (levelCompleted) { game.nrLevel=3; drawLevelCompleteMessage(g); } // Afișează mesajul de felicitări
    ...
    private void drawMessage(Graphics g) {
        if (messageImage != null) {
            ...
            String smallMessage = "Ai grijă la pietre!";
    ...
    public void hideMessage() { showMessage = false; }///ascund mesajul
